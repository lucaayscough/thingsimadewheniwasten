...
if(!$auth->is_authorized()) //user is not authorized - display the login form or lockout text
{
	...
	if(!$auth->is_locked() && isset($login) && $login==false) //user has entered an incorrect password
		echo "<span class='accessDenied'>Password sbagliata. Accesso negato.</span><br/><br/>";
	
	if(!$auth->is_locked() && $auth->get_tries()>0) //show the number of attempts used
		echo "<span class='attemptsDisplay'>".$auth->get_tries()."/".$PowerAuth_maxPasswordAttempts." tenatativi.log in</span><br/><br/>";
	else if(isset($_GET['logout']))
		echo "<span class='accessDenied'>Hai fatto il log out</span><br/><br/>";
	else if(!$auth->is_locked())
		echo "<span class='attemptsDisplay'>Quest pagina e protetta.</span><br/><br/>";
	...
	else //show the login form
	{
		...
		echo "<br/><br/>";
		echo "<input type='checkbox' name='rememberme'/> Ricordami";
		echo "<br/><br/>";
