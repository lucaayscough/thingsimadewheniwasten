...
$utf8_salt = 'Ft!Zp`q4G_~<';
$utf8_bell_salt = 'e?583~';
...
foreach($say['dates'] as $molko => $brian){ //placebo<3
	if(($molko[0] == 'm' && count($brian) != 12) || ($molko[0] == 'd' && count($brian) != 7)){
		echo '<b>Warning</b>: Invalid date parameter values. Date translation disabled.';
		$say['dates'] = FALSE;
		break;
	}
}
...
function utf8_dates($input){
	...
	$m_full = array('January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December');
	$d_short = array('Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun');
	$d_full = array('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday');

	$input = str_replace($m_short, $say['dates'][0], str_replace($m_full, $say['dates'][1], $input));
	$input = str_replace($d_short, $say['dates'][2], str_replace($d_full, $say['dates'][3], $input));
	return $input;
}
